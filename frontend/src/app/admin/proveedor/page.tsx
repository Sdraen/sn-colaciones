...
export default async function ProviderAdminPage() {
  ...
  const [operations, currentMenu, nextMenu, report, notifications, summary] = await Promise.all([
    backendRequestOrNull<ProviderOperationsDto>(`/api/v1/provider/operations?startsOn=${currentStartsOn}`),
    backendRequestOrNull<MenuWeekDto>(`/api/v1/menus/current?startsOn=${currentStartsOn}`),
  ...
  return <ProviderOperationsClient initialOperations={operations} initialCurrentMenu={currentMenu} initialNextMenu={nextMenu} currentStartsOn={currentStartsOn} nextStartsOn={nextStartsOn} initialReport={report} notifications={notifications} initialSummary={summary} />;
}
