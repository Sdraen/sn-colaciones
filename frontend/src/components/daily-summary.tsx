...
export function DailySummary({ initialSummary }: { initialSummary: DailySummaryDto | null }) {
  ...
  return (
    <section className="provider-panel-enter space-y-5" aria-labelledby="daily-summary-title">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          <p className="eyebrow">Producción y despacho</p>
          <h2 id="daily-summary-title" className="mt-1 text-2xl font-black">Resumen completo del día</h2>
        ...
        <div className="flex flex-wrap items-end gap-2 print:hidden">
          <label className="grid gap-1 text-xs font-extrabold text-[var(--muted)]">
            Fecha
            <input className="company-input min-h-11 rounded-xl border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" type="date" value={date} onChange={(event) => setDate(event.target.value)} />
          </label>
          <button type="button" onClick={consult} disabled={loading} className="company-action inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--brand)] px-4 text-sm font-extrabold text-white disabled:opacity-50">
            <Search size={17} /> {loading ? "Consultando…" : "Consultar"}
          </button>
          <button type="button" onClick={() => window.print()} className="company-action inline-flex min-h-11 items-center gap-2 rounded-xl border border-[var(--line)] bg-white px-4 text-sm font-extrabold">
